...
export default class TetrisTile extends PIXI.Container {
    ...
    playBreakAnim() {
        ...
        // 获取纹理
        const texture = this.app.textures['tile' + (this.colorIndex + 1)];
        ...
        for (let i = 0; i < fragmentCount; i++) {
            ...
            // 从缓存池获取粒子
            const sprite = this.playView.getParticle(texture);
            sprite.x = startX;
            sprite.y = startY;
            sprite.alpha = 1;
            sprite.scale.set(startScale);
            sprite.rotation = Math.random() * Math.PI * 2; // 初始随机旋转
            this.playView.addChild(sprite);
            ...
            const tween = new TWEEN.Tween(animObj)
                ...
                .onComplete(() => {
                    // 归还粒子到缓存池
                    this.playView.returnParticle(sprite);
                })
                .start();
        }
    }
